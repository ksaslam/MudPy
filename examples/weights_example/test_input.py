...
'''
#import sys
#sys.path.append('/Users/kaslasm/Documents/projects/mudpy_khurram/MudPy/src/python')
...
########                            GLOBALS                             ########
home='/Users/kaslasm/Documents/projects/mudpy_testing/results/'
...
num_windows=1
reg_spatial=None # np.array([-2]) #np.logspace(-2,1,num=20)#np.logspace(-0.3,0,num=5) #Set to False if you don't want to use it np.array([0.0372759])
...
# Weights related parameters for joint inversion
method_to_min_spread  = 0     # method for optimizing weights = 1 Grid search,= 2 Anealing

static_weights =np.array([0.5,1]) # weights of joint inversion
displacement_weights=np.array([0.5,1]) 
velocity_weights=np.array([1]) 
tsunami_weights=np.array([1]) 
insar_weights=np.array([1])

weights_array=[static_weights,displacement_weights,velocity_weights,tsunami_weights, insar_weights] 
...
if invert==1:
    runslip.run_inversion(home,project_name,run_name,fault_name,model_name,GF_list,G_from_file,
            ...
            nfaults,beta,decimate,bandpass,solver,bounds,weight, weights_array ,method_to_min_spread, Ltype)

view.slip3D('/Users/kaslasm/Documents/projects/mudpy_testing/results/Hawaii_Liu_SM/output/inverse_models/models/GPS_SM.0000.inv')
